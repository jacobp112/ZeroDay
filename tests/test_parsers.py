...
def test_schwab_parser_transactions():
    ...
    # Note: Our basic regex in schwab.py matches dates and creates transactions for lines with "Standard" logic check
    # But in the placeholder implementation I put: if current_date and "Buy" in line
...
def test_fidelity_parser_identification():
    ...
    statement = parser.parse()
    assert statement.account_number == "X12-345678"
...
def test_vanguard_parser_identification():
    ...
    statement = parser.parse()
    assert statement.account_number == "9876-54321"
